...
// スライムのアニメーションデータ
TexAnimData Shadow2::ANIM_DATA[(int)EAnimType::Num] =
{
	{
		new TexAnim[4]
		{
...
// コンストラクタ
Shadow2::Shadow2(int type, const CVector3D& pos)
	: EnemyBase(pos)
	, mp_image(nullptr)
	, m_type(type)
{
...
void Shadow2::ChangeState(EState state)
{
...
void Shadow2::StateIdle()
{
	// 待機アニメーションを再生
	mp_image->ChangeAnimation((int)EAnimType::Idle);
}

void Shadow2::StateAction()
{
	// ステップごとに処理を切り替え
	switch (m_stateStep)
	{
