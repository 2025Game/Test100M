...
// スライムのアニメーションデータ
TexAnimData BGTree1::ANIM_DATA[(int)EAnimType::Num] =
{
	{
		new TexAnim[4]
		{
...
BGTree1::BGTree1(int type, const CVector3D& pos)
	: ObjectBase(pos)
	, mp_image(nullptr)
	, m_type(type)
{
	std::string imagePath;
	if (m_type == 0) imagePath = "treefront.png",	// 画像ファイルのパス
		// プレイヤーの画像を読み込み
...
//int loopN5 = 0;
CVector2D BGTree1::CalcScreenPos(bool grounded) const
{
	CVector2D ret;

	ret.x = ((m_pos.x + (3840 * loopN5)) - (testScrollX * 0.8f));
	...
	if (ret.x < -1920.0f)
	{
		//loopN5++;
		// `loopN` をメンバ変数として更新
		const_cast<BGTree1*>(this)->loopN5++;
	}
...
void BGTree1::ChangeState(EState state)
{
