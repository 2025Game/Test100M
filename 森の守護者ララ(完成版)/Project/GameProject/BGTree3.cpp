...
// スライムのアニメーションデータ
TexAnimData BGTree3::ANIM_DATA[(int)EAnimType::Num] =
{
	{
		new TexAnim[4]
		{
...
BGTree3::BGTree3(int type, const CVector3D& pos)
	: ObjectBase(pos)
	, mp_image(nullptr)
	, m_type(type)
{
	std::string imagePath;
	if (m_type == 0) imagePath = "treeback.png",	// 画像ファイルのパス
		// プレイヤーの画像を読み込み
...
int loopN3 = 0;
CVector2D BGTree3::CalcScreenPos(bool grounded) const
{
	CVector2D ret;

	ret.x = ((m_pos.x + (3840 * loopN3)) - (testScrollX * 0.4f));
	...
	if (ret.x < -1920.0f)
	{
		//loopN3++;
		// `loopN` をメンバ変数として更新
		const_cast<BGTree3*>(this)->loopN3++;
	}
...
void BGTree3::ChangeState(EState state)
{
